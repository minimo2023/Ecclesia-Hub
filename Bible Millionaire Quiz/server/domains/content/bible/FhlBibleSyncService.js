...
export function validateFhlChapterPayload(payload, { book, chapter, sourceVersion } = {}) {
    ...
    const verses = payload.record.map(record => {
        ...
        if (!text) throw new Error(`FHL_BLANK_VERSE:${verse}`);
        seen.add(verse);
        return { verse, text, sectionHeadings, lineBreakAfter, paragraphBreakAfter };
    ...
    async syncBook(pool, {
        ...
        try {
            ...
            for (const download of downloads) {
                for (const row of download.rows) {
                    await client.query(`
                        INSERT INTO bible_verses_v4_staging
                            (id, version, source_version, book, chapter, verse, text, section_headings,
                             line_break_after, paragraph_break_after, upstream_sha256)
                    ...
                    `, [
                        ...
                        row.paragraphBreakAfter, row.hash
                    ]);
                }
            }
            ...
            const invalid = await client.query(`
                ...
                WHERE BTRIM(text) = '' OR chapter < 1 OR verse < 1
            `);
