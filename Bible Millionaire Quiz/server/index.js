...
import { getShare, runRecordingMaintenance } from './domains/scripture-tools/recording-service.js';
import {
...
import { recoverInterruptedSegmentationCruises } from './domains/scripture-tools/segmentation-cruise-service.js';
...
async function bootServer() {
    try {
        ...
        const pausedCruises = await recoverInterruptedSegmentationCruises();
        if (pausedCruises > 0) {
            logger.warn(`⚠️ [ScriptureSegmentation] Paused ${pausedCruises} interrupted cruise(s) at their last completed batch.`);
        }
        ...
        if (process.env.SCRIPTURE_RECORDINGS_ENABLED === 'true') {
            await runRecordingMaintenance();
            setInterval(() => {
                runRecordingMaintenance().catch(error =>
                    logger.warn(`[ScriptureRecording] Maintenance retry required: ${error.message}`)
                );
            }, 60 * 60 * 1000).unref?.();
            logger.info('🎙️ [ScriptureRecording] Private media maintenance registered.');
        }
...
import scriptureRainRoutes from './domains/game/scripture-rain/routes.js';
// content/
...
import appRoutes from './domains/app/routes.js';
...
const SCRIPTURE_RAIN_ENABLED = process.env.SCRIPTURE_RAIN_ENABLED === 'true' || process.env.NODE_ENV !== 'production';
...
if (SCRIPTURE_RAIN_ENABLED) {
    app.use('/api/scripture-rain', scriptureRainRoutes); // 經文雨獨立實驗遊戲
}
...
const builtPublicPath = path.join(__dirname, '../dist');
const snapshotPublicPath = path.join(__dirname, 'public');
const publicPath = fs.existsSync(path.join(builtPublicPath, 'index.html'))
    ? builtPublicPath
    : snapshotPublicPath;
const uploadsPath = path.join(__dirname, 'uploads');
const scriptureToolsPublicPath = path.join(__dirname, '../scripture-tools-app/dist');

const resolveVoiceBlessingPublicOrigin = req => {
    const configuredOrigin = String(process.env.PUBLIC_SITE_ORIGIN || '').trim().replace(/\/+$/u, '');
    if (/^https?:\/\/[^/]+$/iu.test(configuredOrigin)) return configuredOrigin;
    if (config.server.env === 'production') return 'https://xtc-biblestudy.idv.tw';
    return `${req.protocol}://${req.get('host')}`;
};

const serveVoiceBlessingShareShell = async (req, res, next) => {
    try {
        const isMobileRoute = req.path.startsWith('/m/');
        const shellPath = path.join(isMobileRoute ? mobilePublicPath : publicPath, 'index.html');
        const canonicalUrl = `${resolveVoiceBlessingPublicOrigin(req)}/b/${encodeURIComponent(req.params.token)}`;
        let share = null;
        try {
            share = await getShare(req.params.token);
        } catch {
            // The client renders the normal expired/revoked message. Do not expose token details in logs.
        }
        const metadata = buildVoiceBlessingShareMetadata(share, canonicalUrl);
        const shell = await fs.promises.readFile(shellPath, 'utf8');
        const routedShell = absolutizeVoiceBlessingShareAssetUrls(shell, isMobileRoute ? '/m/' : '/');

        res.setHeader('X-Robots-Tag', 'noindex, nofollow, noarchive');
        res.setHeader('Cache-Control', 'private, no-store');
        res.type('html').send(injectVoiceBlessingShareMetadata(routedShell, metadata));
    } catch (error) {
        next(error);
    }
};
...
const builtMobilePublicPath = path.join(__dirname, '../mobile-app/dist');
const snapshotMobilePublicPath = path.join(__dirname, 'public-mobile');
const mobilePublicPath = fs.existsSync(path.join(builtMobilePublicPath, 'index.html'))
    ? builtMobilePublicPath
    : snapshotMobilePublicPath;
