// Socket initialized in socket-client.js or initSocket()
// const socket = io(); // Legacy removed
...
window.selectGame = function (game) {
    if (game === 'reversi') {
        ...
        // Implement Xiangqi launch
        document.getElementById('gameSelector').style.display = 'none';
        document.getElementById('reversiApp').style.display = 'none';
        document.getElementById('chessApp').style.display = 'none';
        if (window.XiangqiGame) {
            window.XiangqiGame.show();
        }
    }
};
...
function switchTurn() {
    ...
    if (!gameState.isGameOver) {
        ...
        if ((gameState.gameMode === 'pve' && gameState.currentPlayer !== gameState.myPlayerId) ||
            ...
            setTimeout(() => {
                if (gameState.isGameOver) return; // double check
                aiTurn();
            }, waitTime);
        }
    }
...
function showScreen(screenName) {
    ...
    if (screenName === 'menu') {
        ...
        // Clear and Hide 3D Board
        if (typeof View3D !== 'undefined') {
            if (View3D.clearBoard) View3D.clearBoard();
            if (View3D.setBoardVisibility) View3D.setBoardVisibility(false);
        }
    } else if (screenName === 'lobby') {
        lobbyScreen.style.display = 'flex';
        if (typeof View3D !== 'undefined' && View3D.setBoardVisibility) {
            View3D.setBoardVisibility(false);
        }
        setTimeout(() => roomIdInput.focus(), 100);
    } else {
        gameScreen.style.display = 'block';
        if (typeof View3D !== 'undefined' && View3D.setBoardVisibility) {
            View3D.setBoardVisibility(true);
        }
    }
}
