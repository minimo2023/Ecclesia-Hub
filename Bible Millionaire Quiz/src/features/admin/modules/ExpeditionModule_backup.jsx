...
import { Save, RefreshCw, Settings, Package, Map, Heart, Clock, Coins, Shield, BookOpen, Tent, Users, Database, AlertCircle, Plus } from 'lucide-react';
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                                ...
                                <div className="space-y-4">
                                    ...
                                    <div className="space-y-3">
                                        <RewardRow label="傳統模式 · 每題智匯金幣" value={classicReward.perQuestion} onChange={val => updateConfig('classic_reward_config', { ...classicReward, perQuestion: val })} />
                                        <RewardRow label="傳統模式 · 勝場分紅(x)" value={classicReward.victoryBonus} step="0.1" onChange={val => updateConfig('classic_reward_config', { ...classicReward, victoryBonus: val })} />
                                        <RewardRow label="極速模式 · 每題智匯金幣" value={speedReward.perQuestion} onChange={val => updateConfig('speed_reward_config', { ...speedReward, perQuestion: val })} />
                                        
                                        <div className="pt-4 border-t border-stone-50 space-y-3">
                                            <h4 className="text-[10px] font-black text-stone-400 uppercase tracking-widest">傳統模式 · 題庫加成</h4>
                                            {(classicReward.categoryBonuses || [
                                                { minBooks: 5, bonus: 0.1 }, 
                                                { minBooks: 11, bonus: 0.15 }
                                            ]).map((b, i) => (
                                                <div key={i} className="flex items-center gap-2">
                                                    <span className="text-[10px] text-stone-400 w-16">≥ {b.minBooks} 卷:</span>
                                                    <input 
                                                        type="number" step="0.01" 
                                                        value={b.bonus} 
                                                        onChange={e => {
                                                            const newBonuses = [...(classicReward.categoryBonuses || [{ minBooks: 5, bonus: 0.1 }, { minBooks: 11, bonus: 0.15 }])];
                                                            newBonuses[i] = { ...newBonuses[i], bonus: parseFloat(e.target.value) || 0 };
                                                            updateConfig('classic_reward_config', { ...classicReward, categoryBonuses: newBonuses });
                                                        }}
                                                        className="w-full bg-stone-50 border border-stone-100 rounded-lg px-2 py-1 text-xs font-mono text-amber-600"
                                                    />
                                                </div>
                                            ))}
                                        </div>
                                        ...
                                        <div className="pt-4 border-t border-stone-50 space-y-3">
                                            <h4 className="text-[10px] font-black text-stone-400 uppercase tracking-widest">極速模式 · 連勝加成</h4>
                                            {(speedReward.streakBonuses || [
                                                { streak: 5, bonus: 0.1 }, 
                                                { streak: 10, bonus: 0.15 }, 
                                                { streak: 15, bonus: 0.2 }
                                            ]).map((b, i) => (
                                                <div key={i} className="flex items-center gap-2">
                                                    <span className="text-[10px] text-stone-400 w-16">≥ {b.streak} 連勝:</span>
                                                    <input 
                                                        type="number" step="0.01" 
                                                        value={b.bonus} 
                                                        onChange={e => {
                                                            const newBonuses = [...(speedReward.streakBonuses || [{ streak: 5, bonus: 0.1 }, { streak: 10, bonus: 0.15 }, { streak: 15, bonus: 0.2 }])];
                                                            newBonuses[i] = { ...newBonuses[i], bonus: parseFloat(e.target.value) || 0 };
                                                            updateConfig('speed_reward_config', { ...speedReward, streakBonuses: newBonuses });
                                                        }}
                                                        className="w-full bg-stone-50 border border-stone-100 rounded-lg px-2 py-1 text-xs font-mono text-purple-600"
                                                    />
                                                </div>
                                            ))}
                                        </div>
